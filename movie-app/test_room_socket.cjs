...
client1.on('connect', () => {
    ...
    step = 1;
    log(`Client 1 joining room: ${roomId}`);
    client1.emit('join-room', roomId, user1);
});

client1.on('user-joined', (user) => {
    log(`Client 1 saw User Joined: ${user.username}`);
});

client1.on('user-left', (data) => {
    log(`Client 1 saw User Left: ${data.username}`);
});

client1.on('member-list', (list) => {
    log(`Client 1 Member List: ${JSON.stringify(list)}`);
});

client2.on('member-list', (list) => {
    log(`Client 2 Member List: ${JSON.stringify(list)}`);
});

client1.on('room-joined', (data) => {
    if (step === 1) {
        log(`Client 1 Joined Room ${roomId}. isHost: ${data.isHost}`);
        if (data.isHost) {
            log('PASS: Client 1 is Host');
            // Step 2: Client 2 joins
            setTimeout(() => {
                step = 2;
                log(`Client 2 joining room: ${roomId}`);
